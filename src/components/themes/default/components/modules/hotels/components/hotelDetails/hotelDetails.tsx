      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div
            ...
            className="relative bg-white rounded-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6"
          >
            <button
              onClick={() => setIsModalOpen(false)}
              className={`absolute top-4 ${locale?.startsWith('ar') ? 'left-4' : 'right-4'
            ...
            <h2
              className={`text-xl font-bold font-urbanist mb-4 ${locale?.startsWith('ar') ? 'text-right' : 'text-left'
                }`}
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {hotelDetails?.amenities?.map((amenity: any, idx: number) => (
                <div
                  key={idx}
                  className={`flex gap-3 items-center ${locale?.startsWith('ar') ? 'flex-row-reverse text-right' : ''
                    }`}
                >
